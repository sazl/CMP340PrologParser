...
class Lexer(object):
    ...
    def tokenize(self, token, value=''):
        return Token(token, value, self.line,
                     span=(self.previous_position, self.position))
    ...
    def raise_error(self):
        self.error = True
        self.errors.append("unknown lexeme: {} line: {} span: {}".format(
            self.current, self.line, (self.previous_position, self.position)))
    ...
    def lex(self):
        ...
        if self.error:
            raise LexicalError(self.errors)
        return self.tokens
